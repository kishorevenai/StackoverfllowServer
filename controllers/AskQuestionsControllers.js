...
const updateVotes = async (req,res) => {
  try {
    ...
    const upIndex = result.upVote.find(id => id === String(userNumber))
    const downIndex = result.downVote.find(id => id === String(userNumber))

    if(value === 'upVote'){
      if(downIndex){
        result.downVote = result.downVote.filter(id => id!== String(userNumber))
      }
      if(!upIndex){
        result.upVote.push(userNumber)
      }
      else{
        result.upVote = result.upVote.filter(id !== String(userNumber))
      }
    }
    else if(value === 'downVote'){
      if(upIndex){
        result.upVote = result.upVote.filter(id => id!== String(userNumber))
      }
      if(!downIndex){
        result.downVote.push(userNumber)
      }
      else{
        result.downVote = result.downVote.filter(id !== String(userNumber))
      }
    }
